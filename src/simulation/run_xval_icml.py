...
from inference.parameters import Parameters
from plotting import plot_prediction_summary, xval_treatments, plot_weighted_theta
from simulation.convenience import Dataset, Decoder, Encoder, SessionVariables, LocalAndGlobal, Objective
from simulation.convenience import Placeholders, TrainingLogData, TrainingStepper, DatasetPair
...
class Runner:
    ...
    def _tidy_args(cls, args, split):
        ...
        print(args)
        if getattr(args, 'HO', None):
            print("Heldout device is %s" % args.HO)
        else:
            args.HO = None # in case not defined at all
    ...
    def _prepare_data(self, data: Dict[str, str]):
        ...
        loaded_data = self.procdata.load_all(data_dir, data["files"], data["devices"])
        np.random.seed(self.args.seed)
        if self.args.HO:
            # We specified a holdout device to act as the validation set.
            d_train, d_val, train_ids, val_ids = procdata.split_holdout_device(self.procdata, loaded_data, self.args.HO)
            train_dataset = Dataset(d_train, train_ids)
            val_dataset = Dataset(d_val, val_ids)
            ...
            val_ids = val_chunks[self.args.split - 1]
            # A DatasetPair object: two Datasets (one train, one val) plus associated information.
            self.dataset_pair = self._decide_dataset_pair(all_ids, val_ids, loaded_data)
    ...
    @staticmethod
    def _decide_dataset_pair(all_ids, val_ids, loaded_data):
        train_ids = np.setdiff1d(all_ids, val_ids)
        train_data, val_data = procdata.split_by_train_val_ids(loaded_data, train_ids, val_ids)
        return DatasetPair(Dataset(train_data, train_ids), Dataset(val_data, val_ids))
    ...
    def set_up(self):
        ...
        model.init_with_params(self.params_dict)
        ...
        variable_summaries(unnormed_iw, 'unn_log_iws')
        variable_summaries(self_normed_iw, 'normed_iws')
        tf.summary.scalar('nonzero_iws', tf.count_nonzero(self_normed_iw))
    ...
    def _run_session(self):
        ...
        held_out_name = self.args.HO or '%d_of_%d' % (self.args.split, self.args.folds)
        ...
        with tf.Session() as sess:
            ...
            print("===========================")
            if self.args.HO:
                split_name = 'heldout device = %s' % self.args.HO
            else:
    ...
    def _evaluate_elbo_and_plot(self, beta_val, epoch, eval_tensors, log_data, merged, sess,
        ...
        if np.mod(epoch, self.args.expensive_test_epoch) == 0:
            print("-------------- EXPENSIVE DEVICE PLOTS --------------")
    ...
    def _plot_prediction_summary_figure(self, dataset, output, epoch, writer):
        ...
        fig = plot_prediction_summary(self.procdata, self.decoder.names, self.dataset_pair.times, dataset.X,
                                               ...
                                               '-')
        plot_op = make_summary_image_op(fig, 'Summary')
        writer.add_summary(plot_op, epoch)
        pp.close(fig)
    ...
    def _plot_weighted_theta_figure(self, training_output, validation_output, valid_writer, epoch, sample):
        ...
        theta_plot_op = make_summary_image_op(theta_fig, name)
        valid_writer.add_summary(theta_plot_op, epoch)
        pp.close(theta_fig)
        pp.close('all')
...
def create_parser(with_split: bool):
    parser = argparse.ArgumentParser(description='ICML PyCRN')
    ...
    parser.add_argument('--expensive_test_epoch', type=int, default=500, help='Frequency of calling expensive test')
    ...
    if with_split:
        # We make --HO (heldout device) and --split mutually exclusive. Really we'd like to say it's allowed
        # to specify *either* --HO *or* --split and/or --folds, but argparse isn't expressive enough for that.
        # So if the user specifies --HO and --folds, there won't be a complaint here, but --folds will be ignored.
        group = parser.add_mutually_exclusive_group()
        group.add_argument('--HO', type=str, help='name of held-out device, e.g. R33S32_Y81C76')
...
if __name__ == "__main__":
    run_on_split(create_parser(True).parse_args())