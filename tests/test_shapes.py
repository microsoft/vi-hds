...
def test_shapes():
    ...
    assert batch.dev_1hot.shape == torch.Size([n_batch, settings.data.device_depth]), "Batch has right shape for 'dev_1hot'"
    assert batch.inputs.shape == torch.Size([n_batch, len(settings.data.conditions)]), "Batch has right shape for 'inputs'"
