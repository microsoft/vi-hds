...
def pre_test(pattern, example, num_folds):
    ...
    if num_folds > 1:
        cmd = ('python vihds/%s.py --seed=0 --folds=%d --experiment=TEST --epochs=4 --test_epoch=2 '
            '--train_sample=10 --test_samples=10 --plot_epoch=0 specs/%s.yaml') % (pattern, num_folds, example)
    else:
        cmd = ('python vihds/%s.py --seed=0 --experiment=TEST --epochs=6 --test_epoch=3 '
            '--train_sample=10 --test_samples=10 --plot_epoch=0 specs/%s.yaml') % (pattern, example)
    print(cmd)
