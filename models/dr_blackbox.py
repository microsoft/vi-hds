...
class DR_Blackbox_RHS(OdeFunc):
    def __init__(self, states : NeuralStates, offset_layer, config, theta, treatments, dev_1hot, precisions=None):
...
class DR_Blackbox(OdeModel):
    def __init__(self, config):
        ...
        self.offset_layer = nn.Linear(self.device_depth, self.n_y)  # Default initializer better than glorot_uniform here
    ...
    def gen_reaction_equations( self, config, theta, treatments, dev_1hot):
        func = DR_Blackbox_RHS(self.neural_states, self.offset_layer, config, theta, treatments, dev_1hot, precisions=self.precisions)
        return func
