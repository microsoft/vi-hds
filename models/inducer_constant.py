...
class Inducer_Constant_Precisions(Inducer_Constant):
    ...
    def initialize_state(self, theta, _treatments):
        ...
        x0 = torch.stack([theta.init_x, theta.init_rfp, theta.init_yfp, zero, zero, theta.init_prec_x, theta.init_prec_rfp, theta.init_prec_yfp, theta.init_prec_cfp], axis=2)
        return x0

    def gen_reaction_equations(self, config, theta, treatments, dev_1hot, condition_on_device=True):
        func = Inducer_Constant_RHS(config, theta, treatments, dev_1hot, condition_on_device, precisions=self.precisions, version=self.version)
        return func
