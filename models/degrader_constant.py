...
class Degrader_Constant_RHS(OdeFunc):
    def __init__(self, config, theta, treatments, dev_1hot, condition_on_device, precisions=None, version=1):
        ...
        # Promoter activity
        self.PBAD = (power(ara,self.nA) + (self.eA  * power(self.KAra,self.nA))) / (power(ara,self.nA) + power(self.KAra,self.nA))
...
class Degrader_Constant(OdeModel):
    def __init__(self, config):
        ...
        self.species = ['OD', 'RFP', 'YFP', 'CFP', 'F530', 'F480', 'LuxR', 'LasR', 'AiiA','C6','C12']
        self.n_species = 11
    ...
    def initialize_state(self, theta, _treatments):
        ...
        x0 = torch.stack([theta.init_x, theta.init_rfp, theta.init_yfp, theta.init_cfp, zero, zero, theta.init_luxR, theta.init_lasR, theta.init_aiiA, c6,c12], axis=2)
        return x0
...
class Degrader_Constant_Precisions(OdeModel):
    ...
    def initialize_state(self, theta, _treatments):
        ...
        x0 = torch.stack([theta.init_x, theta.init_rfp, theta.init_yfp, theta.init_cfp, zero, zero, theta.init_luxR, theta.init_lasR, theta.init_aiiA, c6,c12 ,theta.init_prec_x, theta.init_prec_rfp, theta.init_prec_yfp, theta.init_prec_cfp], axis=2)
        return x0

    def gen_reaction_equations(self, config, theta, treatments, dev_1hot, condition_on_device=False):
        func = Degrader_Constant_RHS(config, theta, treatments, dev_1hot, condition_on_device, precisions=self.precisions, version=self.version)
        self.aR = func.aR
        self.aS = func.aS
        return func
