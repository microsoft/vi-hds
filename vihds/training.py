...
class Training:
    ...
    def cost(
        self, batch_data, batch_results, theta, q, p, full_output=False, writer=None, epoch=None,
    ):
        """Cost function for a VAE model"""
        x_states, x_predict, precisions = batch_results
        log_p_by_species = log_prob_observations(
            self.model, x_predict, batch_data.observations, precisions, self.settings.params.use_laplace,
        )
        ...
        log_unnormalized_iws = (
            log_p_observations + log_p_theta - log_q_theta
        )  # log wi = log [p(x | theta) p(theta) / q (theta | x)]
        ...
        if full_output:
            ...
            output = Results()
            output.init(
                self.model.decoder.state_names, q, theta, elbo, normalized_iws, x_predict, x_states, precisions,
            )
            return output
        else:
    ...
    def _plot_prediction_summary(self, dataset, output, epoch, writer):
        fig = plotting.plot_prediction_summary(
            self.settings.data.devices,
            output.species_names,
            dataset.times,
            dataset.observations.cpu().detach().numpy(),
            output.iw_predict_mu,
            output.iw_predict_std,
            dataset.devices,
            "-",
        )
    ...
    def _plot_species(self, dataset, output, epoch, writer):
        ...
        fig = plotting.species_summary(
            output.species_names,
            dataset.inputs.cpu().detach().numpy(),
            dataset.devices,
            dataset.times,
            output.iw_states,
            devices,
    ...
    def _plot_weighted_theta_figure(self, train_output, valid_output, valid_writer, epoch, sample):
        ...
        fig = plotting.plot_weighted_theta(
            self.encoder.theta_names,
            train_output.normalized_iws,
            train_output.theta,
            self.dataset_pair.train.devices,
            valid_output.normalized_iws,
            valid_output.theta,
            self.dataset_pair.val.devices,
            columns2use=self.settings.params.theta_columns,
            sample=sample,
        )
