...
def create_parser():
    ...
    parser.add_argument('--graph', type=str, default='unnamed', help='Name for the inference graph and also location of tensorboard and saved results of all the nodes in the graph')
    return parser
...
def propagate_params(node,settings,resultmap):
    for incoming in node.incoming:
        print('Incoming node for ' + node.name + ' is ' + incoming.source.name + ' with parameter ' + incoming.sourceParam)
        ...
        for key in keyparams:
            if incoming.targetParam in settings.params[key]:
                print('Target Parameter for ' + node.name + ' is ' + incoming.targetParam + ' which is in ' + key + ' in settings.param')        
                newdist = {}
