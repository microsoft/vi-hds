...
def stat_cuda(msg):
    print("- %-12s  " % (msg + ":"), end="")
    print(
        "|  allocated: %4dM  (max: %4dM)  |  cached: %4dM  (max: %4dM)"
        % (
...
class Results:
    ...
    def init(
        self, species_names, q, theta, elbo, normalized_iws, x_predict, x_states, precisions,
    ):
        """Initialiser"""
        ...
        self.iw_predict_std = np.sqrt(
            np.sum(importance_weights * (x_predict ** 2 + 1.0 / precisions), 1) - self.iw_predict_mu ** 2
        )
